...
class Setup(models.Model):
    ...
    def interval_schedule(self):
        if self.time_interval == TimeInterval.one_min:
            s, c = IntervalSchedule.objects.get_or_create(every=1, period=IntervalSchedule.MINUTES)
            return s
        if self.time_interval == TimeInterval.five_mins:
            s, c = IntervalSchedule.objects.get_or_create(every=5, period=IntervalSchedule.MINUTES)
            return s
        if self.time_interval == TimeInterval.one_hour:
            s, c = IntervalSchedule.objects.get_or_create(every=1, period=IntervalSchedule.HOURS)
            return s
